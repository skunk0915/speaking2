document.addEventListener('DOMContentLoaded', () => {
    ...
    // Sync conversational item audio button UI with playing state
    function syncAudioBtnUI(playing) {
        if (!currentAudioBtn) return;
        const iconPlay = currentAudioBtn.querySelector('.icon-play');
        const iconPause = currentAudioBtn.querySelector('.icon-pause');
        const loader = currentAudioBtn.querySelector('.loader');
        ...
        if (playing) {
            ...
            currentAudioBtn.classList.add('playing');
        } else {
    ...
    function addConversationItem(data, insertAfterGroup = null, isReviewMode = false) {
        ...
        // Sync UI and highlight if this newly created item is currently playing
        if (currentAudio && !currentAudio.paused && !currentAudio.ended && playerTextEl) {
            const cleanText = data.english.trim();
            const playerText = playerTextEl.textContent.trim();
            if (cleanText === playerText || playerText.includes(cleanText)) {
