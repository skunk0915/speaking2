...
if ($mode === 'translation') {
    $prompt = "
    あなたは英語の先生です。
    ユーザーは、以下の「日本語の原文」を英語に翻訳しようとしています。
    ユーザーの翻訳（「ユーザーの英語」）を添削し、さらに自然なネイティブの表現を3つ提案してください。

    重要：
    - これは「会話の返答」ではなく「翻訳」の練習です。ユーザーが原文の主語（私、あなた、彼など）を正しく捉えているか確認してください。
    ...
    - **重要：もしユーザーの英語が、下記の「すでに提示済みの英文例」のいずれかと一致、またはほぼ同じである場合は、それを『間違い』として指摘してはいけません。100%正解として扱ってください。**
    ...
    出力形式（JSONのみ）:
    {
        \"correction\": \"添削コメント（日本語）。提示済みの英文が使われた場合は間違いとして指摘せず、必要に応じてブラッシュアップ案として記述すること。履歴がある場合はそれらを踏まえた内容にすること。無駄な褒め言葉は省略し、解説を重視すること。\",
        \"suggestions\": [
            {
                \"english\": \"自然な表現1 (英語)\",
                \"japanese\": \"その日本語訳\",
    ...
    出力形式（JSONのみ）:
    {
        \"correction\": \"添削コメント（日本語）。提示済みの英文が使われた場合は間違いとして指摘せず、必要に応じてブラッシュアップ案として記述すること。履歴がある場合はそれらを踏まえた内容にすること。無駄な褒め言葉は省略し、解説を重視すること。文脈に合わない場合はその旨を指摘。\",
        \"suggestions\": [
            {
                \"english\": \"自然な表現1 (英語)\",
                \"japanese\": \"その日本語訳\",
