...
- 重要ルール:
  ...
  4. ヘッダーやフッター、サイドバーなどのノイズテキスト（共有メニューや著作権表示など）は完全に無視し、必ずメインの記事本文から抜き出してください。";
    ...
    役割定義:
    - 'japanese': ユーザーが入力した日本語テキストをそのまま（一切変更せずに）格納してください。
    - 'english': 入力された日本語テキスト全体に対する、自然で正確な英訳です。
    
    指示:
    - 'japanese'のキーには、ユーザーの入力『{$japaneseInput}』を「一字一句変えずにそのまま」設定してください。要約、言い換え、分割、省略、疑問文への変更は絶対にしないでください。
    - 'english'のキーには、入力された日本語『{$japaneseInput}』のすべての内容、ニュアンス、感情、詳細を漏れなく含んだ、正確で自然な英訳を生成してください。文字数の制限はありません。元の日本語の意味や長さに合わせて十分な長さで英訳してください。
    - 生成する英語は、以下の基準に従ってください:
      **{$currentLevelInst}**
    - 1人の発話としての翻訳のみを含めてください。複数人の会話形式にしないでください。
    - 前置きや説明、話者名は一切含めず、純粋な英訳のみを出力してください。
    
    出力はJSON形式で、以下のキーを含めてください:
      - 'japanese': ユーザーが入力した日本語テキスト（一字一句変えずにそのまま）
      - 'english': その英訳
      - 'sample_user_answers': この発話に対するユーザーの返答例のリスト（1〜5個程度）。提案数は固定せず、文脈に応じてできるだけ多くのバリエーションを提示してください。ただし、似たような表現ばかりを並めるのは避け、ポジティブ・ネガティブ・質問など様々な視点で提示してください。各要素は 'ja' (日本語) と 'en' (英語) のキーを持つオブジェクトにしてください。";
    } elseif (isset($inputMode) && $inputMode === 'url') {
...
出力はJSON形式で、以下のキーを含めてください:
  ...
  - 'sample_user_answers': その一文に対する会話の返答例のリスト（1〜5個程度）。ポジティブ・ネガティブ・質問など様々な視点で提示してください。各要素は 'ja' (日本語) と 'en' (英語) のキーを持つオブジェクトにしてください。";
    ...
    $pronunciationScores = $input['context']['pronunciation_scores'] ?? '';
    $pronunciationAdvice = $input['context']['pronunciation_advice'] ?? '';
    $isPronunciationQa = $input['context']['is_pronunciation_qa'] ?? false;

    if ($isPronunciationQa) {
        // 発音コーチ専用のプロンプト
        $prompt = "あなたは英語の「発音コーチ」です。ユーザーは提示された英文の発音練習を行い、その評価結果（スコアやAI発音アドバイス）に基づいてあなたに質問をしています。
        
        【最優先の重要ルール】
        - これは「発音（音読）」に関する質問です。作文、文法、スペル、語彙の使い分け、英文のしつこさやオシャレさなど「記述内容の正しさ」に関する解説は【絶対にしないでください】。
        - ユーザーが特定の単語（例：「this」や「that」など）について質問した場合、それはその単語を『発音』する際の発音の難しさ、母音・子音の出し方、舌や口の動かし方、リンキング（隣の音との繋がり）、イントネーション、ストレス（強勢）に関する疑問です。
        - 例えば「thisはなにが駄目なん」という質問であれば、自分が「this」を発音した際のスコアが低かった、もしくはAIから発音の誤りを指摘されたことを指しています。「th」の濁音（/ð/）の舌の挟み方や息の出し方、「i」の短い母音（/ɪ/）の発音方法、あるいは末尾の「s」の無声摩擦音（/s/）など、「this」を正しく発音するための物理的なコツ・改善ポイントを徹底的に解説してください。文法的なダメ出しや「別の単語に置き換えた方が良い」といったアドバイス（例：this を the や that に書き換える提案など）は【厳禁】です。
        
        前提となる発音コンテキスト:
        【対象の英文】: {$englishText}
        【状況/問いかけ】: {$situation}
        【発音評価スコア】: {$pronunciationScores}
        【AIによる発音アドバイス】: {$pronunciationAdvice}
        ...
        指示:
        - ユーザーの質問は「発音」に関するものです。母音・子音の物理的な発音方法（舌の位置、息の出し方など）、リンキング、イントネーション、強勢についてのみ答えてください。
        - 作文や文法、表現方法（言い換えなど）についての解説は一切行わないでください。
        - 日本語での回答は、以下のスタイルを守ってください:
          **{$currentStyleInst}**
        - 必要に応じて発音記号（IPA）やカタカナでの発音イメージ（例：「ズィスではなく、前歯で舌を軽く挟んで...」など）を交えて解説してください。
        - 出力はJSON形式で、以下のキーを含めてください:
          - 'answer': 回答内容（マークダウン形式で記述可）";
    } else {
        // 通常の添削Q&Aプロンプト
        $pronunciationScoresStr = $pronunciationScores ? "\n    【発音評価スコア】: {$pronunciationScores}" : "";
        $pronunciationAdviceStr = $pronunciationAdvice ? "\n    【AIによる発音アドバイス】: {$pronunciationAdvice}" : "";

        $prompt = "あなたは英語学習のアシスタントAIおよび発音コーチです。ユーザーからの質問に答えてください。
        ...
        ($userInput !== 'なし' ? "\n    【ユーザーの回答】: {$userInput}" : "") .
        ($correction !== 'なし' ? "\n    【AIによる添削】: {$correction}" : "") .
        $pronunciationScoresStr .
        $pronunciationAdviceStr . "
        ...
        - 質問が「対象の英文」や「発音」、「AIによる発音アドバイス」に関するものであれば、発音のコツ、リンキング（音の繋がり）、イントネーション、あるいは特定の単語の発音方法について分かりやすく解説してください。
